...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  const logger = createOutputChannelLogger('vyapaka', outputChannel) as StructuredLogger;
  ...
  try {
    ...
    if (config.get('autoDiagnoseOnStartup')) {
      setTimeout(() => runDiagnostics(extensionContext), 1000);
    }

    // Periodic diagnostics
    const interval = config.get('checkInterval') as number;
    if (interval > 0) {
      const timer = setInterval(() => runDiagnostics(extensionContext), interval);
